...
export const AppContextProvider = ({ children }) => {
  ...
  const fetchAllCourses = async () => {
    try {
      ...
      else if (Array.isArray(res.data.data?.courses)) coursesData = res.data.data.courses;
...
const enrollInCourse = async (courseId) => {
  ...
  const calculateChapterTime = (chapter) => {
    ...
    return totalMinutes === 0
      ? "0m"
      : humanizeDuration(totalMinutes * 60 * 1000, { units: ["h", "m"], round: true });
  };
  ...
  const resendOtp = async (email, deliveryMethod = "email") => {
    const res = await axios.post("http://localhost:5000/api/auth/resend-otp", { email, deliveryMethod });
  ...
  const fetchUserEnrolledCourses = async () => {
    ...
    try {
      ...
      const mergedCourses = progressData.map((p) => {
        ...
        return { ...course, progress: { totalTopics, completedTopics, overallProgress, isCompleted } };
      });
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      const coursesRes = await axios.get(`http://localhost:5000/api/courses/teacher/${user._id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const myCourses = coursesRes.data?.data?.courses || [];

      const totalEarnings = myCourses.reduce(
        (acc, course) => acc + (course.price || 0) * (course.enrollmentCount || 0),
        0
      );
      ...
      const enrolledStudentsData = progressData.map((p) => {
        ...
        return { student: p.student || { name: "Unknown", thumbnail: "/default-profile.png" }, title: course.title || "Unknown Course", createdAt: p.enrolledAt };
      });

      setDashboardData({ totalCourses: myCourses.length, myCourses, totalEarnings, enrolledStudentsData });
    } catch (err) {
      console.error("Dashboard fetch failed:", err);
      setDashboardData({ totalCourses: 0, myCourses: [], totalEarnings: 0, enrolledStudentsData: [] });
    } finally {
  ...
  const fetchQuizzes = async () => {
    try {
      ...
      else if (Array.isArray(res.data.data?.quizzes)) quizzesData = res.data.data.quizzes;
  ...
  const addQuiz = async (quiz) => {
    try {
      const res = await axios.post("http://localhost:5000/api/quizzes", quiz, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const fetchAdminUsers = async (params = {}) => {
    setLoading(true);
    try {
      const res = await axios.get("http://localhost:5000/api/admin/users", { headers: { Authorization: `Bearer ${token}` }, params });
  ...
  const toggleUserStatus = async (id) => {
    const res = await axios.patch(`http://localhost:5000/api/admin/users/${id}/toggle-status`, {}, { headers: { Authorization: `Bearer ${token}` } });
    return res.data.data;
  };

  // ===== Effects =====
  useEffect(() => { fetchAllCourses(); }, []);
  useEffect(() => { if (allCourses.length > 0) fetchUserEnrolledCourses(); }, [allCourses]);
  useEffect(() => { if (token) fetchUserEnrolledCourses(); }, [token]);
  useEffect(() => { if (token) fetchProfile(); }, [token]);
  useEffect(() => { if (token && user?._id) fetchDashboardData(); }, [token, user]);
