import React, { useContext } from 'react';
import { AppContext } from '../../context/AppContext';
import { NavLink } from 'react-router-dom';
import { assets } from '../../assets/assets';

const Sidebar = () => {
  const { isTeacher } = useContext(AppContext);

  const menuItems = [
    { name: 'Dashboard', path: '/teacher', icon: assets.home_icon },
    { name: 'Add Course', path: '/teacher/add-course', icon: assets.add_icon },
    { name: 'My Courses', path: '/teacher/my-courses', icon: assets.my_course_icon },
    { name: 'Student Enrolled', path: '/teacher/student-enrolled', icon: assets.person_tick_icon },
    { name: 'Add Quiz', path: '/teacher/add-quiz', icon: assets.addQuiz_icon },
    { name: 'Messages', path: '/teacher/messages', icon: assets.messages_icon },
  ];

  return isTeacher && (
    <div className='md:w-64 w-16 border-r min-h-screen text-base border-gray-500 py-2 flex flex-col'>
      {menuItems.map((item) => (
        <NavLink
          to={item.path}
          key={item.name}
          end={item.path === '/teacher'}
          className={({ isActive }) =>
           `flex items-center md:flex-row flex-col md:justify-start justify-center py-3.5 md:px-10 gap-3 ${
      isActive
        ? 'bg-indigo-50 border-r-[6px] border-indigo-500/90'
        : 'hover:bg-gray-100/90 border-r-[6px] border-white hover:border-gray-100/90'
    }`
          }
        >
          <img src={item.icon} alt={item.name} className='w-6 h-6' />
          <p className='md:block hidden text-center'>{item.name}</p>
        </NavLink>
      ))}
    </div>
  );
};

export default Sidebar;
